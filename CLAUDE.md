...
- **`store.js`** — the single source of truth. Holds the in-memory DB (`persons`, `relations`, `tags`), persists it to `localStorage`, and runs schema migrations on load (`migrate()`) so older saved data gains new fields without a version bump. Exposes derived-data helpers (`childrenOf`, `partnerOf`, `grandparentsOf`, `ageOf`/`ageText`, etc.) that the rest of the app relies on instead of reading `db` directly. Data model notes:
  ...
  - A birthday needs day **and** month, which can come from either a full `birth.date` or from `birth.day`/`birth.month` when no year is known (then `date` and `year` stay null and the person simply has no derivable age). Always go through `birthDayMonthOf()` / `hasBirthday()` rather than reading `birth.date` directly — that is the mistake to avoid here. Deceased people are excluded. `birthdayReminder` defaults to `false` on every person.
...
- **`calendar.js`** — writes birthdays as yearly recurring all-day events into a **dedicated secondary calendar the app creates itself** (id in `Settings.calendarId`, user-editable name). **This is the reminder mechanism**: a static PWA cannot wake itself at a point in time (the Notification Triggers API was abandoned by Chrome) and real Web Push requires an application server to sign/send with VAPID keys — there is no backend here by design. The calendar app does the reminding instead. Details that matter:
  - Scope is `calendar.app.created` and nothing else. Its authoritative description is *"Make secondary Google calendars, and see, create, change, and delete events **on them**"* — calendars and their events only. **Do not trust the scope lists on the API reference pages**: `calendarList.patch` lists this scope but rejects it with `401 Invalid Credentials` in practice. Setting the calendar's colour would need `calendar.calendarlist` on top, which grants sight of the user's whole calendar list — too much for cosmetics, so **colour is deliberately not a feature**; the user sets it in Google Calendar directly.
  - Cosmetics must never block the reminders. Events are synced **first**, the rename after, and the rename is fail-soft: it returns a warning instead of throwing. An earlier version applied name/colour first and a failing colour patch aborted the whole sync, so not a single event was ever created.
  - A value that failed to apply is not recorded in `calendarAppliedName`, so the next sync retries it; matching values issue no redundant PATCH.
  - If the calendar is deleted in Google, the next sync recreates it and clears every `calendarEventId`, since those IDs are then worthless.
  - Google counts reminder lead time in minutes before midnight of the event day, so `calendarLeadDays` is converted as `days * 1440 - 540` (09:00 on that day), with 0 meaning midnight.
...
- **`ics.js`** — minimal `.ics` reader for the birthday import in Settings, deliberately not a full RFC 5545 parser: it unfolds continuation lines, unescapes `\,`/`\;`/`\n`, and returns `{summary, year, month, day}` per `VEVENT`, skipping entries that carry a `RECURRENCE-ID` (those are single-instance overrides of a series already present, and would otherwise duplicate the person). `nameFromSummary()` splits the title verbatim — first word first name, remainder last name — with **no** filtering of filler words like "Geburtstag"; that was tried and removed, because guessing produced surprises. The import creates every person plainly: no preview, no duplicate detection, reminder left off, and one mandatory user-chosen category per run.
