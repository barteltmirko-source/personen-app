...
- **`calendar.js`** — writes birthdays as yearly recurring all-day events into a **dedicated secondary calendar the app creates itself** (id kept in `Settings.calendarId`; user-editable name and colour). **This is the reminder mechanism**: a static PWA cannot wake itself at a point in time (the Notification Triggers API was abandoned by Chrome) and real Web Push requires an application server to sign/send with VAPID keys — there is no backend here by design. The calendar app does the reminding instead. Details that matter:
  - Scope is `calendar.app.created`, deliberately *not* the broader `calendar.events`: it authorises exactly what is needed (create a secondary calendar, rename it, recolour it, manage its events) and grants no access to the user's other calendars. It covers `calendars.insert`, `calendars.patch` **and** `calendarList.patch`.
  - The colour lives on the *calendarList* entry, not the calendar — patched with `?colorRgbFormat=true` so any hex works instead of Google's fixed palette; the foreground colour is derived from the background's luminance.
  - `calendarAppliedName`/`calendarAppliedColor` record what was last pushed, so a routine sync issues no redundant PATCHes.
